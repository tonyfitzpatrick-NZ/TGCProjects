...
export default function ScheduleAdminPanel({ activeTab, onRefresh }) {
  ...
  return (
    <div>
      ...
      {activeTab === 'options' && data.length > 0 && (
        <div>
          {data.map((row, i) => (
            <div key={i} style={{ padding: '16px', border: '1px solid #e2e8f0', borderRadius: '10px', marginBottom: '12px', background: '#fff' }}>
              ...
              <span style={{ color: '#666' }}>{row.option_label}</span>
        ...
        <div>
          {data.map(item => (
            <div key={item.id} style={{ padding: '16px', border: '1px solid #e2e8f0', borderRadius: '10px', marginBottom: '12px' }}>
              {item.label} <span style={{ color: '#888' }}>(CBI: {item.cbi_code})</span>
              <button onClick={() => deleteItem('sched_items', item.id)} style={{ float: 'right', color: 'red' }}>Delete</button>
            </div>
          ))}
        </div>
      )}
      ...
      <p style={{ marginTop: '40px', color: '#888', textAlign: 'center' }}>
        Full edit forms (CBI, descriptions, links, product data) coming in next update.
      </p>
    </div>
  );
}
