...
export default function ScheduleAdminPanel({ activeTab = 'options' }) {
  ...
  async function loadData() {
    ...
    try {
      if (activeTab === 'options') {
        ...
        setData([]); // placeholder for other tabs
      }
  ...
  if (activeTab !== 'options') {
    return <div style={{ padding: '40px', textAlign: 'center', color: '#666' }}>Coming soon: {activeTab} management</div>;
  }
    ...
    <div>
      ...
      {data.map((row, index) => (
        <div key={index} style={{ padding: '18px 22px', border: '1px solid #e2e8f0', borderRadius: '12px', marginBottom: '14px', position: 'relative', background: '#fff' }}>
          ...
          {row.sched_item_option_assignments?.length > 0 && (
